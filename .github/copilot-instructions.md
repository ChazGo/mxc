...
- **Experimental config-floor catalog**: `sdk/node/src/config-floors/catalog.json` is a reviewed, read-only proof-of-concept catalog whose entries embed the existing `SandboxPolicy` vocabulary with source-only symbols. Keep it aligned with `sdk/node/src/config-floors/schema.json`; `scripts/versioning/validate-config-floors.js` gates the schema shape, while the TypeScript resolver validates semantic references and rejects unsafe paths and unsupported fields. Config floors are compatibility input, never authorization or a host-policy override. See [`docs/sandbox-config-floors-poc.md`](../docs/sandbox-config-floors-poc.md).
